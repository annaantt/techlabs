...
tweets = tweepy.Cursor(api.search_tweets,q="bitcoin", until=d).items(count)
...
# Run this version if database is empty
df.to_csv('database.csv', mode='a', header=True)

# Run this version if database already has header + existing data
